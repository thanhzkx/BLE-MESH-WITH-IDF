...
#define SOIL_ADC_CHANNEL ADC1_CHANNEL_4  // GPIO4
#define SENSOR_PROPERTY_ID_SOIL_MOISTURE  0x2A58  // Tạm dùng ID user-defined
...
static esp_ble_mesh_model_t *sensor_server_model;
...
NET_BUF_SIMPLE_DEFINE_STATIC(sensor_data_2, 1);  // Độ ẩm từ DHT11
NET_BUF_SIMPLE_DEFINE_STATIC(sensor_data_3, 1);
...
static int dht11_read_bit() {
    while (gpio_get_level(DHT_GPIO) == 0); // đợi lên 1
    delay_us(30); // đợi 30us
    return gpio_get_level(DHT_GPIO); // nếu còn 1 => bit 1, else 0
}
...
static uint8_t dht11_read_byte() {
    ...
    for (int i = 0; i < 8; i++) {
        byte <<= 1;
        byte |= dht11_read_bit();
        while (gpio_get_level(DHT_GPIO)); // chờ về 0
    }
    return byte;
}

// Hàm chính đọc nhiệt độ & độ ẩm
bool dht11_read(int *humidity, int *temperature) {
    ...
    vTaskDelay(pdMS_TO_TICKS(20)); // giữ mức thấp > 18ms
    gpio_set_level(DHT_GPIO, 1);
    delay_us(40);
    gpio_set_direction(DHT_GPIO, GPIO_MODE_INPUT);

    // Check response
    if (gpio_get_level(DHT_GPIO) == 1) return false;
...
static esp_ble_mesh_sensor_state_t sensor_states[4] = {
    ...
    [2] = {
    .sensor_property_id = 0x0068, // Relative Humidity
    .descriptor = {
        .positive_tolerance = SENSOR_POSITIVE_TOLERANCE,
        .negative_tolerance = SENSOR_NEGATIVE_TOLERANCE,
        .sampling_function = SENSOR_SAMPLE_FUNCTION,
...
[3] = {
    .sensor_property_id = SENSOR_PROPERTY_ID_SOIL_MOISTURE,
    .descriptor = {
        .positive_tolerance = SENSOR_POSITIVE_TOLERANCE,
        .negative_tolerance = SENSOR_NEGATIVE_TOLERANCE,
        .sampling_function = SENSOR_SAMPLE_FUNCTION,
...
static void publish_sensor_data(void)
{
    ...
    uint8_t status[10]; // 5 bytes mỗi cảm biến, 2 cảm biến = 10 byte
    int i;

    for (i = 0; i < ARRAY_SIZE(sensor_states); i++) {
        uint8_t fmt = ESP_BLE_MESH_SENSOR_DATA_FORMAT_A;
        //uint8_t len = sensor_states[i].sensor_data.length + 1;
        uint16_t prop_id = sensor_states[i].sensor_property_id;

        // MPID Format A = 2 byte: 1 nibble length, 1 nibble format, 12 bits property id
        uint16_t mpid = ((fmt & 0x0F) << 12) | ((sensor_states[i].sensor_data.length & 0x0F) << 8) | (prop_id & 0xFF);
        status[total_len++] = mpid & 0xFF;
        status[total_len++] = (mpid >> 8) & 0xFF;

        // Append sensor value (1 byte)
        status[total_len++] = sensor_states[i].sensor_data.raw_value->data[0];
    }

    err = esp_ble_mesh_model_publish(sensor_server.model, ESP_BLE_MESH_MODEL_OP_SENSOR_STATUS, total_len, status, ROLE_NODE);
    if (err != ESP_OK) {
...
static void prov_complete(uint16_t net_idx, uint16_t addr, uint8_t flags, uint32_t iv_index)
{
    ...
    if (dht11_read(&humi, &temp)) {
        sensor_data_0.data[0] = temp;         // Indoor temp
        sensor_data_1.data[0] = temp + 5;     // Outdoor temp (fake)
        sensor_data_2.data[0] = humi;         // Humidity
        ESP_LOGI(TAG, "Read DHT11 OK: Temp = %d °C, Humi = %d%%", temp, humi);
    } else {
...
static esp_err_t ble_mesh_init(void)
{
        ...
        sensor_server_model = &elements[0].sig_models[1]; // index 1 là SENSOR_SRV
...
// --- Định nghĩa trước ---
static void sensor_publish_task(void *arg);
static esp_ble_mesh_model_t *sensor_server_model = NULL;
int read_soil_moisture_percent() {
    int adc_val = adc1_get_raw(ADC1_CHANNEL_4);  // GPIO4
    int moisture = 100 - ((adc_val * 100) / 4095);
...
void app_main(void)
{
    ...
    adc1_config_channel_atten(ADC1_CHANNEL_4, ADC_ATTEN_DB_11);  // phù hợp 0~3.3V
